...
class Main:
    ...
    LED_GEEL_PIN: int = 8
    LED_ROOD_PIN: int = 7
    LED_STATUSSEN: list = [0, 70, 90]
    LED_STOPLICHT: list = [{'led_pin': LED_GROEN_PIN}, {'led_pin': LED_GEEL_PIN}, {'led_pin': LED_ROOD_PIN}]

    # Globale variabelen
    _aankomst_snelheid: int = 0
    _aantal_personen_in_systeem: int = 0
    _betreden_knop_waarde: float = 0
    _betreden_knop_status: bool = False  # Huidige knopstatus
    _vorige_knop_status: bool = False   # Vorige knopstatus
    ...
    def start_main_loop(self) -> None:
        ...
        while True:
            ...
            if huidige_betreden_knop_waarde >= 0.5 and vorige_betreden_knop_waarde < 0.5:
            ...
            if self._is_aankomst_snelheid_interval_over():
                ...
                print(f"Nieuwe bezettingsgraad: {self._wachtrijTheorie.krijg_bezettings_graad()}%")
                print(f"Nieuwe wachttijd: {self._wachtrijTheorie.krijg_wacht_tijd()}")
